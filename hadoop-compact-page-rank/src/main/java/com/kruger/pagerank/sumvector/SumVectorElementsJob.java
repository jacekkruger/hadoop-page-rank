...
public class SumVectorElementsJob {

	public static Job createJob(Configuration conf) throws IOException {
		Job job = Job.getInstance(conf, "Sum vector elements");
		job.setJarByClass(SumVectorElementsJob.class);
		job.setMapperClass(SumVectorElementsMapper.class);
		job.setReducerClass(SumVectorElementsReducer.class);
		...
		FileOutputFormat.setOutputPath(job,
				new Path(conf.get(PageRank.DATA_PATH), "vector" + conf.get(PageRank.ITERATION_CURRENT) + "sum"));
